...
class RelationModel(object):
    ...
    def update(self, batch):
        ...
        # one_hot_labels = self.one_hot_embedding(labels, num_classes=41)
        one_hot_labels = F.one_hot(labels, num_classes=-1).type(torch.float32)
        one_hot_labels = one_hot_labels[:, :41]
        if self.opt['cuda']:
    ...
    def predict(self, batch, unsort=True):
        ...
        probs = (F.sigmoid(logits) * inputs['supplemental']['relation_masks'][0]).data.cpu().numpy().tolist()
        # Only include typed relations
